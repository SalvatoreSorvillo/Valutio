...
Persisted financial validation also covers finite account balances (negative values are allowed for credit-card/current accounts) and ownership shares, non-negative asset values, goal targets/savings, recurring amount/kind/start/share, debt balance/APR/payment, and tax thresholds, income, tax paid, deductions, carried losses, rates, discount months, bracket caps/rates, and adjustments. Apply the field's positive/non-negative rule explicitly rather than relying on HTML input constraints.
